...
class Top(Elaboratable):
    ...
    def elaborate(self, platform):
        ...
        with m.FSM():
            ...
            load_rd = InsI(self.insn).rd
            with m.State("lw.delay"):
                with m.If(mmu.read.valid):
                    m.d.sync += self.write_xreg(load_rd, mmu.read.value)
                    m.next = "fetch.init"

            with m.State("lh.delay"):
                with m.If(mmu.read.valid):
                    m.d.sync += self.write_xreg(load_rd, mmu.read.value[:16].as_signed())
                    m.next = "fetch.init"

            with m.State("lhu.delay"):
                with m.If(mmu.read.valid):
                    m.d.sync += self.write_xreg(load_rd, mmu.read.value[:16])
                    m.next = "fetch.init"

            with m.State("lb.delay"):
                with m.If(mmu.read.valid):
                    m.d.sync += self.write_xreg(load_rd, mmu.read.value[:8].as_signed())
                    m.next = "fetch.init"

            with m.State("lbu.delay"):
                with m.If(mmu.read.valid):
                    m.d.sync += self.write_xreg(load_rd, mmu.read.value[:8])
                    m.next = "fetch.init"
